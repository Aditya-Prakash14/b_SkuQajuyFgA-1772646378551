...
| 3 | **code done, gateway flag-gated** | `c54d43f`, 0031 · gateway `480f4e9` | `payments` ledger, written by a trigger whenever `payment_status` flips to paid (cash checkbox, CRM mark-paid, webhook) · `paid_at`, `cancelled_at` / `cancelled_by` / `cancellation_reason`; customer cancels record the customer, any other cancel is attributed by trigger (0033: `ops` when an admin, else `system`) · `invoices` own-row SELECT + private `invoices` bucket · Receipt screen (GST split, share, CRM invoice PDF by signed URL) · **Razorpay, built and deployed but off:** `razorpay-create-order` (JWT; reads the booking as the user, amount from the row, writes the ledger row, answers 503 until `RAZORPAY_KEY_ID` / `RAZORPAY_KEY_SECRET` are set) · `razorpay-webhook` (HMAC-verified; captured → ledger + `payment_status = 'paid'` + a "Paid online" timeline note that becomes the push; failed; refund.processed → refunded) · the website's `/pay` page runs Checkout and returns to the app · app: "Pay now by UPI or card" at checkout and on an unpaid booking, a pay step on the confirmation screen, live paid state on tracking — all behind `ONLINE_PAYMENTS_ENABLED = false` in `src/lib/payments.ts`. **To switch on:** set the three function secrets (incl. `RAZORPAY_WEBHOOK_SECRET`), register `…/functions/v1/razorpay-webhook` in the Razorpay dashboard for `payment.captured`, `payment.failed`, `refund.processed`, deploy the website, flip the flag. Tips charged online and API-initiated refunds stay for after that. |
...
| 5 | **5.6 done** (§11 default "yes"); rest waits on §11 | 0033 | Prime Now requests can be rated and tipped: `reviews.request_id`, `submit_request_review()`, one per request; `my_stats()` and the helper card count request reviews and tips; the partner's earnings screen shows tips. Membership, coupons, pincodes/map, live location, job photos, B2B enquiry, referrals, Hindi — decisions first. |
...
2. **Database changes are a migration file in `supabase/migrations/NNNN_*.sql` AND applied live.** Apply with the Supabase MCP `apply_migration` (same SQL), then verify with a query. The next number is `0034`.
...
| 0033 | `reviews.request_id` + `submit_request_review()` · `my_stats()` / `my_booking_helper()` count request reviews · `record_cancellation()` triggers attribute every cancel | 5.6, 3 | applied |
| 0034+ | memberships / coupons / service_areas / helper_locations / job-photos as decided | 5 | — |

Edge functions deployed: `notify-customer` (v1, Vault secret), `delete-account` (v1, JWT), `razorpay-create-order` (v1, JWT, 503 until keys), `razorpay-webhook` (v1, HMAC, 503 until secret). Sources in `supabase/functions/`. Redeploy with the Supabase MCP `deploy_edge_function` after editing. Function secrets are set with `supabase secrets set NAME=value` (or the dashboard) — never in SQL or the repo.
...
11. **Prime Now rating** — allow rating a request. *Default: yes — done (0033, `480f4e9`).*
