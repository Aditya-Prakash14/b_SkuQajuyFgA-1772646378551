...
import { dateParts, formatINR, upcomingDays } from '../../lib/format'
import { SLOTS, TASK_LABEL, createPrimeNowRequest, type SlotId } from '../../lib/prime-now'
  ...
  async function submit() {
    ...
    try {
      const res = await createPrimeNowRequest({
        ...
        notes: notes || null,
        timing,
        // A scheduled visit is booked for the morning of the chosen day; the
        // exact hour is confirmed on the call.
        scheduledFor: timing === 'scheduled' && day ? new Date(day.setHours(10, 0, 0, 0)).toISOString() : null,
      })
    ...
    <Screen edges={[]}>
      <ScrollView contentContainerStyle={{ padding: 22, paddingBottom: 32, gap: 16 }}>
        ...
        <View className="flex-row gap-3">
          {(
            [
              ...
              { id: 'scheduled', label: 'Schedule', sub: 'Pick a day' },
            ] as const
          ).map((o) => {
        ...
        <Card>
          ...
          {defaultAddress ? (
            <>
              <Text className="mt-1.5 font-bold text-[15px] text-foreground">{defaultAddress.label ?? 'Home'}</Text>
